...
(
    `lock_id`           number(20) not null comment '主键id',
    `trans_id`          number(20) not null comment '全局事务id',
    `participant_id`    number(20) not null comment 'hmily参与者id',
...
(
    `undo_id`         number(20) not null comment '主键id',
    `partcipant_id`   number(20) not null comment '参与者id',
    `trans_id`        number(20) not null comment '全局事务id',
    `resource_id`     varchar(256) not null comment '资源id，at模式下为jdbc url',
    `undo_invocation` BLOB     not null comment '回滚调用点',
...
(
    `participant_id`     number(20)  not null comment '参与者事务id',
    `participant_ref_id` number(20)  not null comment '参与者关联id且套调用时候会存在',
    `trans_id`           number(20)  not null comment '全局事务id',
    ...
    `retry`              int(3) default 0 not null comment '重试次数',
    ...
    `confirm_invocation` BLOB      null comment 'confirm调用点',
    `cancel_invocation`  BLOB      null comment 'cancel调用点',
