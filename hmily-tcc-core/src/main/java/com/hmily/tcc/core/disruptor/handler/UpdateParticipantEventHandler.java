 ...
 */
@Component
public class UpdateParticipantEventHandler implements EventHandler<HmilyTransactionEvent> {

    private final CoordinatorService coordinatorService;

    @Autowired
    public UpdateParticipantEventHandler(CoordinatorService coordinatorService) {
    ...
    public void onEvent(final HmilyTransactionEvent hmilyTransactionEvent, final long sequence, final boolean endOfBatch) {
        if (hmilyTransactionEvent.getType() == EventTypeEnum.UPDATE_PARTICIPANT.getCode()) {
