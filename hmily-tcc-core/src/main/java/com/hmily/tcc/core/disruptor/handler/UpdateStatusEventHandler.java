 ...
 */
@Component
public class UpdateStatusEventHandler implements EventHandler<HmilyTransactionEvent> {

    private final CoordinatorService coordinatorService;

    @Autowired
    public UpdateStatusEventHandler(CoordinatorService coordinatorService) {
    ...
    public void onEvent(final HmilyTransactionEvent hmilyTransactionEvent, final long sequence, final boolean endOfBatch) {
        if (hmilyTransactionEvent.getType() == EventTypeEnum.UPDATE_STATUS.getCode()) {
